...
Object.defineProperties( module.exports = Abstract, {
  ...
  createModel: {
    ...
    value: function createModel( Model ) {
      ...
      // @todo Add automatic "create" method naming based on instantiator's method's name (if set);

      // Set custom name for creating method if available
      var _create = Model.get( 'create_method', 'create' );

      if( !_create || 'string' !== typeof _create ) {
        _create = 'create';
      }

      // Context wrapper for instantiation
      Model[ _create ] = function createInstance() { return Abstract.createInstance.apply( Model, arguments ); }
